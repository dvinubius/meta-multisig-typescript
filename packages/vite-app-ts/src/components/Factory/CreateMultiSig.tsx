...
const CreateMultiSig: FC = () => {
  ...
  const updateName = (ev: SyntheticEvent): void => {
  ...
  const addOwnerField = (): void => {
    const newOwners = [...owners, ''];
    setOwners(newOwners);
    setOwnersErrors(newOwners.map((o) => ''));
  };

  const removeOwnerField = (idx: number): void => {
    const newOwners = [...owners];
  ...
  const updateOwner = (value: string, idx: number): void => {
    const newOwners = [...owners];
    newOwners[idx] = value;
    setOwners(newOwners);
              ...
              <div key={idx}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  ...
                  <AddressInput
                    ...
                    onChange={(v): void => updateOwner(v.toString(), idx)}
                  ...
                  {idx > 1 && (
                    <Button
                      ...
                      danger
                      onClick={(): void => removeOwnerField(idx)}>
                      <DeleteOutlined />
                    </Button>
                  )}
                </div>
