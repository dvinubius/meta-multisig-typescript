...
export const AddressInput: FC<IAddressInputProps> = (props) => {
    ...
    async (newValue: string) => {
      ...
      if (newValue != null) {
        ...
        if (props.showError) {
          ...
          if (!isAddress) {
            setFormatError('Invalid Address');
          }
        }
      }
    },
  ...
  return (
    <div style={{ ...props.wrapperStyle, display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
      {scanner}
      <Input
      ...
      />
      {formatError && <div style={{ color: errorColor }}>{formatError}</div>}
    </div>
  );
};
